...
export default function BillGrid({ bills, policyAreas }: Props) {
  ...
  return (
    <div className="min-h-screen" style={{ background: "#0d1117" }}>
      {stateParam && <div style={{ background: "#21262d", borderRadius: "4px", padding: "8px 14px", display: "flex", alignItems: "center", gap: "12px", flexWrap: "wrap", maxWidth: "520px", margin: "0 1rem 0 1rem" }}><span style={{ fontSize: "14px", color: "#dc2626", flexShrink: 0 }}>📍</span><span style={{ fontSize: "13px", color: "#8b9198" }}>Filtered to <span style={{ color: "#e6edf3", fontWeight: 600 }}>{stateParam}</span> — <span style={{ color: "#f5c518", fontWeight: 700 }}>{filtered.length.toLocaleString()} abandoned bills shown below.</span> Your representatives introduced every one. None survived.</span></div>}1px solid #DDC9B4", borderRadius: "6px", padding: "10px 16px", margin: "1rem", fontSize: "14px", color: "#111", lineHeight: 1.6 }}><span style={{ fontWeight: 700, color: "#dc2626" }}>Showing abandoned bills from {stateParam} representatives.</span> Your elected officials introduced these bills — and let every one die. No hearing. No vote. No explanation.</div>}
